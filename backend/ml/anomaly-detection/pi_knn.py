...
for n in [5, 10, 15, 20, 25, 30, 35, 40]:
    ...
    num_train_malicious = np.sum(train_y_pred == 1)
    num_train_benign = np.sum(train_y_pred == 0)
    print(f"[Train Set] Number of malicious flows: {num_train_malicious}")
    print(f"[Train Set] Number of benign flows: {num_train_benign}")
